...
from MPCInputConSoftOutputCon import LMPCcompute_inputcons_outputSoftCons
...
z_min_error = np.array([-1000, ref - 10, -1000, -1000])
z_min_safe = np.array([0, 0, 0, 0])
z_max = np.array([ref + 10, ref + 1000, ref + 1000, ref + 1000])
...
rk = ref * np.ones((N_pre * ydim, 1))
dk = d_bar[:, 0]
uk = np.zeros((udim, 1))
us = np.zeros((udim,))
sk = np.tile(us[0], (N_pre * udim, 1))
...
for k in range(N - 1):
    ...
    # 3) MPC controller
    # ---------------------------------------------------------------------
    uk = LMPCcompute_inputcons_outputSoftCons(
        rk, xk, dk, uk, Ad, Bd, Bd_d, Css, u_min, u_max, u_delta_min, u_delta_max, 
        z_min, z_max, N_pre, Q_cof, u_delta_cof, eta_cof1, eta_cof2
    )
